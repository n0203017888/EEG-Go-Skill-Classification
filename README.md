...
除了分類，也嘗試用頂葉區的 EEG 特徵**迴歸預測行為指標**，**三者都失敗**。

> ⚠️ 迴歸實驗用的**不是**分類實驗那組特徵。分類用的是時間軸平均後的 PLV + BE 經 PCA 降維
> （session-level，168 筆樣本）；迴歸用的是保留時間解析度的 **190 維特徵**
> （Parietal 9 通道 × 38 個時間點 × 5 頻帶，trial-level，數千筆樣本），不做 PCA。
> 兩者同樣取自頂葉 EEG，但特徵表示與樣本粒度都不同。
...
| | 分類 | 迴歸 |
|---|---|---|
| 預測目標 | 棋力等級 — **穩定的個人屬性** | RT / 勝率 / 勝步 — **逐 trial 的即時波動** |
| 結果 | Acc 0.946 | 全部失敗 |

即使迴歸端用了時間解析度更高、樣本量大上數十倍的特徵，仍然預測不了任何一個即時指標。
這指向一個一致的解釋：**頂葉 EEG 攜帶的是個體的長期專業度特徵，而非當下的表現狀態。**

---

## 資料集

資料集含 57 位三個 session 皆完整的受試者（amateur 28 / master 21 / prof 8）、5,096 trials、
500 Hz、32 通道、每 trial 21 秒，分 ss01 開局 / ss02 中盤 / ss03 尾盤三個 session，
格式為 EEGLAB `.set` + `.fdt`。

分類實驗實際使用 **56 人**（amateur 28 / master 20 / prof 8）：一位 master 的 ss01 原始檔
命名不符批次特徵萃取的樣式而未被處理，該受試者因此未納入。
故 session-level 樣本數為 56 × 3 = **168**。
